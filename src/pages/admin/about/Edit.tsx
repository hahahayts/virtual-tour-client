...
import type { AboutCreateSchema } from "@/schema/about";
...
const EditAbout = () => {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (formData: z.infer<typeof AboutCreateSchema>) =>
      updateData(formData, "1", "about"),
    onSuccess: () => {
      toast.success("About Info updated successfully", { richColors: true });
      navigate("/admin/about/1");
    },
    onError: (error) => {
  ...
  if (isFetching) {
    return <DataFetching name="About" />;
  }

  if (error) {
    return <ErrorEdit name="About" />;
  }

  if (!data && !isFetching) {
    return <ErrorFetchingData name="History" url="about" />;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 p-6">
      <Header
        ...
        description="Edit the about Tubigon entry below. Click save when you're done."
        backUrl="about/1"
      />
