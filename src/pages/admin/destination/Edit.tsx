...
import { z } from "zod";
import { destinationSchema } from "@/schema/destination";
...
const EditDestination = () => {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (formatData: z.infer<typeof destinationSchema>) =>
      updateData(formatData, id, "destinations"),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["destinations", id] });
      toast.success("Destination updated successfully", { richColors: true });
  ...
  if (isFetching) {
    return <DataFetching name="destination" />;
  }

  // Handle error state
  if (error) {
    return <ErrorEdit name="destination" />;
  }

  // Handle case where destination is not found
  if (!data && !isFetching) {
    return <ErrorFetchingData name="Destination" url="destinations" />;
  }
  return (
    <div className="max-w-4xl mx-auto space-y-6 p-6">
      <Header
        title="Edit Destination"
        description="Edit destination details. Click save when you're done."
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[1, 2, 3, 4, 5].map((num) => (
                  <div key={num} className="space-y-2">
                    ...
                    <Input
                      {...register(
                        `imageUrl_${num}` as keyof typeof destinationSchema.shape
                      )}
                      id={`imageUrl_${num}`}
