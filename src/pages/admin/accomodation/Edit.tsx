...
const EditAccommodation = () => {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (formatData: z.infer<typeof AccommodationSchema>) =>
      updateData(formatData, id, "accommodations"),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["accommodations", id] });
      toast.success("Accommodation updated successfully", { richColors: true });
  ...
  if (isFetching) {
    return <DataFetching name="accommodation" />;
  }

  // Handle error state
  if (error) {
    return <ErrorEdit name="accommodation" />;
  }

  // Handle case where destination is not found
  if (!data && !isFetching) {
    return <ErrorFetchingData name="Accommodation" url="accommodations" />;
  }
  return (
